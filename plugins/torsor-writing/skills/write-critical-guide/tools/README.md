...
**Check convergence after each build.** A document can have coupled numbering rules,
so a fixed number of passes is not a guarantee. If the build reports unresolved references
whose labels exist, run it again and inspect the log until the references settle.
