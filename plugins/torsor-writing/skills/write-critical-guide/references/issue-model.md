...
**An invented category example.** Suppose an extended version of the averaging manuscript
includes a program that computes projection matrices. If several issues concern its handling
of empty inputs, exact arithmetic, and output dimensions, a `computational` category can give
those corrections a separate home. The examples concern implementation behavior and ask the
author to change the program. They are independent of the exposition and bibliography.

Choose a separate category only when it helps someone act on a substantial group of issues.
...
an author. The qualifier makes that distinction explicit.
...
Copy it out of the `.tex` file, macros and all — `The kernel of $P$ has dimension $n$.`,
as in the fictional fixture, rather than a transcription of the rendered mathematics.
Retyping from the PDF can produce an anchor that never matches.
...
**Every example value in this file is synthetic.** A critical guide's contents — the paper,
its authors, its passages, and above all the findings against it — are confidential to the
person who commissioned it. Never paste a real paper's title, authors, identifier, or quoted
text into documentation, into a commit message, or into any repository that is shared. Invent
an example. The manuscript and an executable ledger are in `tests/fixtures/review/` at
the repository root; the schema below shows additional optional fields.

```yaml
paper:
  title: "Averaging on coordinate spaces"             # fictional fixture
  authors: ["A. Author", "B. Coauthor"]
  identifier: "synthetic-example"
  source: "paper.tex"                               # relative to the ledger
  submitted: "paper.pdf"                            # optional — compile the fixture
  locations_refer_to: "the fictional source `paper.tex`"
...
issues:
  - id: M-1
    ...
    grade: major
    section: "A statement that is wrong as printed"
    location: "Kernel calculation"
    anchor: 'The kernel of $P$ has dimension $n$.'
    caption: "Kernel dimension"  # short label; index and note caption
    quote: >
      The kernel of $P$ has dimension $n$.
    note: >
      The image is the span of the all-ones vector and has dimension one.
      Rank-nullity therefore gives kernel dimension $n-1$.
    request: >
      Replace $n$ with $n-1$ and give the rank-nullity calculation.
    depends_on: []                                  # e.g. [M-2]
    ...
    grade_note: ""                                  # optional qualifier
    also: []                                        # IDs of other defined issues to bundle

verified_clean:
  - source: "Fixture lemma: constant vectors"
    statement: "The image of the averaging map is the span of the all-ones vector."
    used_for: "the corrected kernel calculation"
...
Markdown. For example, two adjacent spelling corrections can share one box while
remaining separate entries in the ledger.
...
math still pairs correctly: `"constant on $\mathbf{Q}^n$"` comes out as one
...
the table into a full entry, keeping its tag.
