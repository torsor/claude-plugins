...
**It distinguishes evidence from judgment.** State the calculation, quotation, or source
comparison that supports an assessment, then state what that evidence establishes. The
document's producer does not speak in the first person; see the family stance.
...
author. The worked examples in `${CLAUDE_PLUGIN_ROOT}/assets/commons/stance.md` use a
fictional averaging map. "Rank-nullity gives kernel dimension $n-1$" states the reason for
the correction. "I checked the dimension" leaves that reason unstated.

**Present tense for the paper; past for a completed check.** "The paper assigns dimension
$n$ to the kernel" describes its printed claim. "The image calculation was checked directly"
records a completed examination. Prefer the calculation itself when it can be stated briefly.
...
without, it does not address, the paper compresses, the calculation gives, the argument supplies, one
