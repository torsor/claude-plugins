...
Templates for the `critical-guide/` directory. Adapt the marked fields and verify the
resulting package with its build checks.
...
# `showonlyrefs` with keytheorems `sharenumber=equation` can require this,
# where statement numbers depend on which equations the previous pass referenced
# and additional passes may be needed. Each `build` runs three. Drop the second
...
The package generates as much as it carries. Ship this beside the Makefile, and keep the two
lists in step: **everything `make clean` removes belongs here.**
...
# Generated by the guide build on the way to 00-guide.pdf
.guide-combined.md
00-guide.tex
...
`00-guide.tex` and `.guide-combined.md` are easy to miss, since the guide only started
producing them when its build went to four steps. Check the pair after any change to `clean`:

```sh
for f in $(sed -n '/^clean:/,/^$/p' Makefile | grep -o '[*.][A-Za-z_.-]*'); do
  git check-ignore -q "x$f" || echo "not ignored: $f"
done
```
...
long document. Grep the built PDF's text, which is what
`check_guide.py` does.
