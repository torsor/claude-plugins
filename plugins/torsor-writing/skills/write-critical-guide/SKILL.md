...
**Check the allegation against competing explanations.** A claim may already carry its
reference at another location, use different hypotheses, or depend on an earlier source
that both works cite. Compare the actual statements and their chronology before assigning
an attribution issue. Record a refuted candidate privately, with the reason it was dropped.

**Verification effort follows consequence.** A claim about a missing credit or a defective
main argument needs an independent check of its premises. Repairing an argument may also
expose an error in the guide's own ledger. Correct that ledger before using it as evidence.
...
3. `check_guide.py` reports `00-guide.pdf` `ok`. **The guide gets the same mechanical gate as
   ...
   pandoc may not relay LaTeX's warnings. Unresolved citations can therefore survive
   a visual spot-check; inspect the generated text and build log.
...
one this package recorded for an earlier run. If you did not measure something, say so.
