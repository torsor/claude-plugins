...
cannot process every citation macro; `\mbox` can protect one but prevents a line break inside
its argument. Keep surrounding prose outside that box. Put the issue tag at the end of the
removed phrase so it does not force a break in the middle. Check the resulting paragraph's
line breaks in both builds.
