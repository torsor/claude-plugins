...
them**. Keep this distinction explicit in the triage ledger.

**Consolidate absences rather than enumerating them.** Group related omissions into a
statement that names the affected passages and the missing work. In a fictional extension
of the averaging example, separate weighted-projection claims might all lack the same image
calculation. State that shared omission once and list the affected claims.
...
apparent branch may disappear once a proposed route is checked and found invalid.
...
anything touching a hypothesis or a statement. Each check must examine the proposed mathematics
and its consequences in the paper. Agreement based only on the ledger is not verification.
...
Two repairs can each be plausible alone and inconsistent together. In an invented extension
of the averaging fixture, a definition might admit zero-length vectors while a later formula
divides by the vector length. Restricting the definition to positive lengths also changes any
example that invokes the empty vector. Decide the linked changes together and check every use.

Keep the disposition of an interacting group consistent across the marked and clean builds.
...
a direct comparison with the source and with every hypothesis used in the repair.
