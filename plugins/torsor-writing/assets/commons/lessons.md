...
**A number in context is not a measurement.** A duration quoted in an earlier report
does not measure the current run. Record a timestamp at phase boundaries and compute elapsed
time from those timestamps. File modification times can corroborate when outputs were written,
but need not measure the entire run. If no adequate timing record exists, say so.

Apply the same rule to token counts, costs, page counts, and similarity measurements. Name the
measurement and its source; never adopt a number from another document as a new observation.
