...
commit-stamped. Write "As of `<commit>`, the room has verified X for the stated inputs; Y is
...
> In this invented example, as of `<commit>`, the room has checked the averaging
> projection on vectors of lengths two and three. A proof for every positive length is
> still open in the room's ledger. The direct calculation suggests testing idempotence
> before attempting the weighted variant.
