...
| Voice | third person, timeless ("the authors prove") | third person, timeless | **present-tense, dated, commit-stamped** ("as of `<commit>`, the room has verified X") |
...
- **Attribute to the room and the moment, not timeless authors.** "As of `<commit>`, the room
  has verified X for the stated inputs; Y is staged for referee; Z is open." Present-tense,
...
- *State register (invented example):* "As of `<commit>`, the room has checked the
  averaging map on vectors of lengths two and three. Its ledger still marks a proof
  for every positive length as open. Checking idempotence is the next proposed step."
