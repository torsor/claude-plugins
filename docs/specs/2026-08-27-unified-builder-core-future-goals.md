# Design note — a shared document builder

This document describes a possible future build architecture. The current release uses
per-document scaffolds. Earlier implementation work is retained in the maintainer's private
archive; the present repository does not ship or promise a working unified builder.
...
- **Tier 3 — a different artifact class** (`write-critical-guide`, `write-paper-repairs`).
  These operate on an **external** paper: critical-guide runs `annotate_tex.py` and
